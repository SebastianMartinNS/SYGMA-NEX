...
from sigma_nex.core.retriever import (
    _get_model, build_index, search_moduli, get_moduli, Retriever,
    DATA_PATH, INDEX_PATH, MAPPING_PATH
)
...
class TestRetrieverRealistic:
    ...
    def test_faiss_operations_real(self):
        ...
        with patch('sigma_nex.core.retriever.faiss') as mock_faiss, \
            ...
            expected_texts = ["Modulo1 :: Descrizione modulo 1",
    ...
    def test_search_index_functionality_real(self):
        ...
        for query in test_queries:
            try:
                ...
                expected_errors = ['faiss', 'index', 'moduli', 'file not found', 'transformers']
...
class TestRetrieverErrorHandling:
    ...
    def test_missing_data_files_handling_real(self):
        ...
        with patch('sigma_nex.core.retriever.faiss.read_index', side_effect=FileNotFoundError):
            try:
    ...
    def test_corrupted_data_handling_real(self):
        ...
        # Test JSON corrotto
        with patch('sigma_nex.core.retriever.get_moduli', side_effect=json.JSONDecodeError("Invalid", "", 0)):
...
class TestRetrieverIntegration:
    ...
    def test_retriever_search_algorithms_coverage(self):
        ...
        with patch('sigma_nex.core.retriever.faiss') as mock_faiss, \
            ...
            # Mock mapping file
            mock_mapping = {"0": {"title": "Test Module 1"}, "1": {"title": "Test Module 2"}}
