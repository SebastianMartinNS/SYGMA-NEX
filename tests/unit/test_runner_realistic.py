...
class TestRunnerRealistic:
    ...
    def test_send_with_progress_real(self, test_config):
        ...
        with patch('subprocess.Popen') as mock_popen:
            mock_process = Mock()
            mock_process.communicate.side_effect = subprocess.TimeoutExpired('ollama', 300)
    ...
    def test_interactive_repl_real(self, test_config):
        ...
        with patch('click.prompt', side_effect=inputs), \
            ...
            stats_calls = [call for call in mock_echo.call_args_list
        ...
        with patch('click.prompt', side_effect=EOFError), \
            ...
            goodbye_calls = [call for call in mock_echo.call_args_list
    ...
    def test_self_heal_file_real(self, test_config):
        ...
        try:
            ...
            with patch.object(runner, '_send_with_progress', return_value="Fixed code") as mock_send, \
...
class TestRunnerAdvancedFunctionality:
    ...
    def test_process_query_real(self, test_config):
        ...
        with patch('sigma_nex.core.runner.translate_it_to_en', return_value="test query en"), \
             patch('sigma_nex.core.runner.translate_en_to_it', return_value="risposta test"), \
    ...
    def test_process_query_translation_pipeline_real(self, test_config):
        ...
        with patch('sigma_nex.core.runner.translate_it_to_en') as mock_it_en, \
        ...
        with patch('sigma_nex.core.runner.translate_it_to_en', side_effect=Exception("Translation error")), \
             patch('sigma_nex.core.runner.translate_en_to_it', side_effect=Exception("Translation error")), \
             patch('sigma_nex.core.runner.build_prompt', return_value="test prompt"), \
             patch.object(runner, '_call_model', return_value="model response"):
    ...
    def test_runner_context_integration_real(self, test_config):
        ...
        with patch('sigma_nex.core.runner.build_prompt') as mock_build_prompt, \
             ...
             patch('sigma_nex.core.runner.translate_en_to_it', return_value="it response"):
    ...
    def test_runner_subprocess_operations_coverage(self, test_config):
        ...
        try:
            ...
            with patch('subprocess.run') as mock_subprocess, \
                 patch('sigma_nex.utils.validation.validate_file_path', return_value=True):
